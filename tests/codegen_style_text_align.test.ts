...
describe("M7 text_align style codegen", () => {
  it("emits lv_obj_set_style_text_align from style", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "forgeui-style-align-"));
    ...
    expect(screenC).toContain("lv_obj_set_style_text_align");
    expect(screenC).toContain("LV_TEXT_ALIGN_CENTER");
  });
});
