...
| 控件缩放 | 选中后 **8 手柄**（四角 + 四边中点，对标 Beken）；拖拽改 `frame.x/y/w/h`（方向见下表），最小边长 16；锁定无手柄；位移按 `canvasView.zoom` 换算 | FR-012 |
...
| **文本 chrome（硬性）** | 样式键 `text_font_size`→`fontSize`（默认 14）且 **CodeGen 选字**；`text_font`→注册字体族；按钮 `.btn-label` **width:100%** + 生成子 label `LV_PCT(100)`；对标 Beken 字号/对齐/长文本 | FR-016e-b |
| **设备框** | `.screen` 宽高 = `project.display.width/height`；居中于深色舞台；框内为近似渲染区 | FR-021c |
...
| 方向 | 改写字段 | 光标 |
|------|----------|------|
| `br` 右下 | `w`, `h` | nwse-resize |
| `tr` 右上 | `y`, `w`, `h` | nesw-resize |
| `bl` 左下 | `x`, `w`, `h` | nesw-resize |
| `tl` 左上 | `x`, `y`, `w`, `h` | nwse-resize |
| `mr` 右 | `w` | ew-resize |
| `ml` 左 | `x`, `w` | ew-resize |
| `bc` 下 | `h` | ns-resize |
| `tc` 上 | `y`, `h` | ns-resize |

实现：`apps/designer/src/utils/resize-handles.ts` + `WidgetView.vue`；测试 `tests/designer_resize_handles.test.ts`。

**旋转锚点（对标 Beken / 常见设计器）：**

| 项 | 契约 |
|----|------|
| 形态 | 选中未锁定时，顶边中点上方一根竖线 + **圆形旋转手柄内含旋转箭头图示**（对标 Beken；与 8 缩放方块区分） |
| 枢轴 | 控件几何中心（与 CodeGen `transform_pivot` 50%/50%、画布 `transform-origin: center` 一致） |
| 交互 | 拖拽旋转手柄改 `frame.rotation`（度，整数）；松手入历史；属性面板「旋转 (°)」双向同步 |
| 约束 | `locked` / 编辑禁用时不显示；角度归一化到 `[0, 360)` |
| 实现 | `apps/designer/src/utils/rotate-handle.ts` + `WidgetView.vue`；测试 `tests/designer_rotate_handle.test.ts` |

**Selection chrome 分层（对标 Beken `CanvasComponent`，V1.6i）：**

| 项 | 契约 |
|----|------|
| 外壳 | `.widget`：仅几何（`left/top/w/h` + `rotation`）+ 事件；**固定 `overflow: visible`** |
| 内容 | `.widget-body`：背景/文字/padding/`long_mode`·`clip_corner` 等 **overflow**；子树在此层 |
| 选中框 | 独立 `.selection-border`（非依赖内容裁切露边）；缩放点纯色方块；旋转柄挂外壳 |
| 层级 | 选中 `z-index: 200`；拖/缩/转 `300`（避免被兄弟控件盖住） |
| 设备框 | `.screen` 不对 selection 裁切；舞台 `.stage` 仍 `overflow: hidden` |
...
|------|------|-----------|------|----------|
| 1 | **屏幕信息** / **位置信息** | 宽/高（无 3×3 格） | X/Y/W/H + **3×3 容器方位格**（吸附父容器）/ 旋转 / 布局 | `updateScreen` / `updateNode.frame` |
...
**位置信息 3×3 格（对标 Beken，契约详见属性面板模块详设 §3.5）：**

- 每格示意点位于该格对应角/边/心，禁止九格同图。  
- 点击 = 将选中控件 **frame** 吸附到**直接父容器**九宫方位，并写 `anchorX/Y`；不得只改枢轴不移动。  
- Core API：`alignFrameToParent`；写回仍走 `updateNode.frame`。
