...
| 消费方 | 入口 | 说明 |
|--------|------|------|
| CLI / Electron Main / 单测 | `@forgeui/core` | 完整 barrel（含 `validateProjectDir`、`openProject` 等 Node I/O） |
| Designer **Renderer** | `@forgeui/core/widgets`、`/types`、`/frame-anchor` | **仅**无 `fs`/`createRequire` 的子路径；禁止 barrel（详设 §2 V1.27） |

`validate.ts` 使用 `createRequire` 加载 Ajv，**不可**进入 Vite 浏览器图。
...
interface ProjectModelApi {
  ...
  /** 改父或同级插位；newParentId=null 视为 screen 根；新父须 isContainer；禁成环；改父后 clamp frame */
...
interface WidgetSpec {
  ...
  isContainer: boolean;            // true 才可作为 addChildNode / moveNode 的父；当前：screen/container/tabview/tileview/win/menu（button=false）
