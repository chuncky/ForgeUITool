...
> **Beken 规格：** `ref/beken/.../component-specs/spinner/`（若存在）
...
| 字段 | 面板标签 | 类型 | 默认值 |
|------|----------|------|--------|
| `anim_time` | 动画时长(ms) | `number` | `1000` |
| `arc_length` | 弧长(°) | `number` | `60` |
...
*分册序号：16 · 生成自注册表与用户手册；模块架构见 [总目录](../嵌入式UI工具_控件属性面板详细设计说明.md)。*
