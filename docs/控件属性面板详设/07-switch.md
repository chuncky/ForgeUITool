...
> **Beken 规格：** `ref/beken/.../component-specs/switch/`（若存在）
...
添加时由 `WidgetSpec.defaultStyle`（`STYLE_SEED_SWITCH`）写入，对齐 LVGL `theme_default` Light，保证画布与模拟一致。
...
*分册序号：7 · 生成自注册表与用户手册；模块架构见 [总目录](../嵌入式UI工具_控件属性面板详细设计说明.md)。*
