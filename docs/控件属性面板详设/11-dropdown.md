...
> **Beken 规格：** `ref/beken/.../component-specs/dropdown/`（若存在）
...
| 字段 | 面板标签 | 类型 | 默认值 |
|------|----------|------|--------|
| `options` | 选项（每行一项） | `text` | `"One\nTwo\nThree"` |

### 2.2 扩展数据（extraData）

- 编辑器种类：`items`（见模块详设 §4.2 / `ExtraDataGroup`）
- 默认：`{ items: [{ text: "One" }, { text: "Two" }, { text: "Three" }] }`
...
*分册序号：11 · 生成自注册表与用户手册；模块架构见 [总目录](../嵌入式UI工具_控件属性面板详细设计说明.md)。*
