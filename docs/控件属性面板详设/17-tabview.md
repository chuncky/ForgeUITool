...
> **Beken 规格：** `ref/beken/.../component-specs/tabview/`（若存在）
...
| 字段 | 面板标签 | 类型 | 默认值 |
|------|----------|------|--------|
| `tab_bar_size` | 标签栏尺寸 | `number` | `50` |
| `tab_bar_position` | 标签栏位置 | `enum` | `"TOP"` |

### 2.2 扩展数据（extraData）

- 编辑器种类：`tabs`（见模块详设 §4.2 / `ExtraDataGroup`）
- 默认：`{ tabs: [{ name: "Tab 1" }, { name: "Tab 2" }], selectedTabIndex: 0 }`
...
*分册序号：17 · 生成自注册表与用户手册；模块架构见 [总目录](../嵌入式UI工具_控件属性面板详细设计说明.md)。*
