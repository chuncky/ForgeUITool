...
> **Beken 规格：** `ref/beken/.../component-specs/table/`（若存在）
...
- 编辑器种类：`cells`（见模块详设 §4.2 / `ExtraDataGroup`）
- 默认：`{ cells: [ ["Header 1", "Header 2"], ["Cell 1", "Cell 2"], ["Cell 3", "Cell 4"], ], }`
...
*分册序号：34 · 生成自注册表与用户手册；模块架构见 [总目录](../嵌入式UI工具_控件属性面板详细设计说明.md)。*
