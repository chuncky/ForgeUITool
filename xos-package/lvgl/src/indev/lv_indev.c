...
#define LV_INDEV_DEF_SCROLL_LIMIT        5
...
/*Custom long press time for user events in milliseconds
 *Time to send `LV_EVENT_USER_LONG_PRESSED` (1.5 seconds)*/
#ifdef LV_CONFIG_USER_LONG_PRESS_TIME
#define LV_INDEV_DEF_USER_LONG_PRESS_TIME      LV_CONFIG_USER_LONG_PRESS_TIME
#else
#define LV_INDEV_DEF_USER_LONG_PRESS_TIME      1500
#endif
...
static void indev_keypad_proc(lv_indev_t * i, lv_indev_data_t * data)
{
    ...
    /*Pressing*/
        ...
        if(i->long_pr_sent == 0 && lv_tick_elaps(i->pr_timestamp) > i->long_press_time) {
            ...
            if(data->key == LV_KEY_ENTER || data->key == LV_KEY_RESET || 
               data->key == LV_KEY_F3 || data->key == LV_KEY_F2) {
                i->longpr_rep_timestamp = lv_tick_get();
                if(send_event(LV_EVENT_LONG_PRESSED, indev_act) == LV_RESULT_INVALID) return;
            }
        }
        /*Custom long press for LV_KEY_F2 (1.5 seconds)*/
        if(data->key == LV_KEY_F2) {
            if(i->user_long_pr_sent == 0 && lv_tick_elaps(i->pr_timestamp) > LV_INDEV_DEF_USER_LONG_PRESS_TIME) { /* 1.5 seconds */
                i->user_long_pr_sent = 1;
                if(is_enabled) {
                    if(send_event(LV_EVENT_USER_LONG_PRESSED, indev_act) == LV_RESULT_INVALID) return;
                }
            }
        }
        /*Long press repeated time has elapsed?*/
            ...
            if(data->key == LV_KEY_ENTER || data->key == LV_KEY_F3 || data->key == LV_KEY_F2) {
    ...
    /*Release happened*/
        ...
        else if((data->key == LV_KEY_F3 || data->key == LV_KEY_F2) && (i->long_pr_sent == 0) && (i->user_long_pr_sent == 0)) {
            if(send_event(LV_EVENT_SHORT_CLICKED, indev_act) == LV_RESULT_INVALID) return;
        }
        /*Long press release for LV_KEY_F2*/
        else if(data->key == LV_KEY_F2 && i->user_long_pr_sent == 1) {
            if(send_event(LV_EVENT_USER_LONG_PRESSED_RELEASE, indev_act) == LV_RESULT_INVALID) return;
        }
        indev_hook_handler(i, data);
        i->pr_timestamp = 0;
        i->long_pr_sent = 0;
        i->user_long_pr_sent = 0;
    }
    indev_obj_act = NULL;
}
...
static void indev_proc_press(lv_indev_t * indev)
{
    ...
    if(indev_obj_act) {
        ...
        //LV_LOG_USER("x=%d y=%d px=%d py=%d", indev->pointer.vect.x, indev->pointer.vect.y, indev->pointer.act_point.x, indev->pointer.act_point.y);
        if(indev_act->wait_until_release) return;
...
static void indev_proc_release(lv_indev_t * indev)
{
    if (indev->pointer.act_obj)
        indev_proc_press(indev); //avoid last press event lost
    ...
    if(indev_obj_act) {
        ...
        if(is_enabled) {
            if(scroll_obj == NULL) {
                if (indev->pointer.gesture_dir == LV_DIR_NONE) {
                    if(send_event(LV_EVENT_NO_GESTURE_CLICKED, indev_act) == LV_RESULT_INVALID) return;
                }
...
__attribute__((weak)) int indev_hook_send_event_handler(lv_indev_t * indev, lv_event_code_t code, void * param)
{
    (void)indev; (void)code; (void)param;
    return 0;
}

static lv_result_t send_event(lv_event_code_t code, void * param)
{
    if(indev_hook_send_event_handler(indev_act, code, param)) {
        indev_reset_check(indev_act);
    ...
    if(code == LV_EVENT_PRESSED ||
       ...
       code == LV_EVENT_NO_GESTURE_CLICKED ||
