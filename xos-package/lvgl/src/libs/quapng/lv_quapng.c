...
static lv_draw_buf_t *decode_png_file(const char *filename, uint32_t width, uint32_t height);
...
static lv_result_t decoder_open(lv_image_decoder_t * decoder, lv_image_decoder_dsc_t * dsc)
{
    ...
    if(dsc->src_type == LV_IMAGE_SRC_FILE) {
        ...
        lv_draw_buf_t * decoded = decode_png_file(fn, dsc->header.w, dsc->header.h);
        if(decoded == NULL) {
...
static lv_draw_buf_t *decode_png_file(const char *filename, uint32_t width, uint32_t height)
{
    ...
    decoded = lv_draw_buf_create(width, height, LV_COLOR_FORMAT_ARGB8888, width * 4);
    if(decoded == NULL) {
        LV_LOG_ERROR("alloc PNG_IMAGE_SIZE(%" LV_PRIu32 ") failed: %s", width * height * 4, filename);
        lv_free(data);
        return NULL;
    }

    ret = lv_quapng_decode(data, data_size, decoded->data);
    lv_free(data);
    if(ret) {
