...
lv_draw_buf_t * lv_draw_buf_create(uint32_t w, uint32_t h, lv_color_format_t cf, uint32_t stride)
{
    ...
    //LV_LOG_USER("Create buf w=%u h=%u stride=%u data_size=%u draw_buf=%p", w, h, stride, draw_buf->data_size, draw_buf);
    return draw_buf;
}
                                    ...
                                    uint32_t stride)
{
    ...
    if(size > draw_buf->data_size) {
        LV_LOG_USER("Draw buf too small for new shape w=%u h=%u size=%u data_size=%u draw_buf=%p", w, h, size, draw_buf->data_size, draw_buf);
        return NULL;
    }
...
void lv_draw_buf_destroy(lv_draw_buf_t * buf)
{
    ...
    //LV_LOG_USER("Destroy buf w=%u h=%u stride=%u data_size=%u draw_buf=%p", buf->header.w, buf->header.h, buf->header.stride, buf->data_size, buf);
...
static uint32_t width_to_stride(uint32_t w, lv_color_format_t color_format)
{
    ...
    width_byte = ALIGNTO(w, 8) * lv_color_format_get_bpp(color_format);
#else
