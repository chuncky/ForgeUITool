                                  ...
                                  const lv_area_t * coords, lv_draw_image_core_cb draw_core_cb)
{
    ...
    if(res != LV_RESULT_OK) {
        ...
        if(src_type == LV_IMAGE_SRC_FILE) {
            LV_LOG_ERROR("Failed to open image src=%s type=file", (const char *)draw_dsc->src);
        }
        else {
            LV_LOG_ERROR("Failed to open image src=%p type=%d", draw_dsc->src, (int)src_type);
        }
        return;
    }
                                 ...
                                 const lv_area_t * coords, lv_draw_image_core_cb draw_core_cb)
{
    ...
    if(res != LV_RESULT_OK) {
        lv_image_src_t src_type = lv_image_src_get_type(draw_dsc->src);
        if(src_type == LV_IMAGE_SRC_FILE) {
            LV_LOG_ERROR("Failed to open tiled image src=%s type=file", (const char *)draw_dsc->src);
        }
        else {
            LV_LOG_ERROR("Failed to open tiled image src=%p type=%d", draw_dsc->src, (int)src_type);
        }
        return;
    }
