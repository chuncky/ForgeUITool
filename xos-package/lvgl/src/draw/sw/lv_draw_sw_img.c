...
#if defined(CONFIG_XOS_HW_TRANSFORM_IMAGE) && (!defined(BUILD_SIMULATOR) || BUILD_SIMULATOR == 0)
                          ...
                          const lv_area_t * img_coords, const lv_area_t * clipped_img_area)
{
...
#if defined(CONFIG_XOS_HW_TRANSFORM_IMAGE) && (!defined(BUILD_SIMULATOR) || BUILD_SIMULATOR == 0)
        if (lv_quagl_adpt_draw_image(draw_dsc, decoder_dsc, draw_unit, &blend_dsc, clipped_img_area) == 0)
            return;
#endif
        lv_draw_sw_blend(draw_unit, &blend_dsc);
    }
    ...
    /* check whether it is possible to accelerate the operation in synchronouse mode */
...
#if defined(CONFIG_XOS_HW_TRANSFORM_IMAGE) && (!defined(BUILD_SIMULATOR) || BUILD_SIMULATOR == 0)
        if (transformed && !masked && draw_dsc->recolor_opa <= LV_OPA_MIN) {
            blend_dsc.src_area = (lv_area_t *)img_coords;
            if (lv_quagl_adpt_draw_image(draw_dsc, decoder_dsc, draw_unit, &blend_dsc, clipped_img_area) == 0)
                return;
#if 0
            printf("blend_dsc.src_area [%d, %d, %d, %d] blend [%d, %d, %d, %d]\n", blend_dsc.src_area->x1, blend_dsc.src_area->x2, blend_dsc.src_area->y1, blend_dsc.src_area->y2,
                blend_dsc.blend_area->x1, blend_dsc.blend_area->x2, blend_dsc.blend_area->y1, blend_dsc.blend_area->y2);
            printf("blend_dsc.src_stride %d src_color_format %d src_size %d mask_buf %p mask_res %d mask_stride %d blend_mode %d\n",
                blend_dsc.src_stride, blend_dsc.src_color_format, blend_dsc.src_size,
                blend_dsc.mask_buf, blend_dsc.mask_res, blend_dsc.mask_stride, blend_dsc.blend_mode);
#endif
        }
        /*In the other cases every pixel need to be checked one-by-one*/
        //printf("img_coords [%d, %d, %d, %d] clipped_img_area [%d, %d, %d, %d]\n", img_coords->x1, img_coords->x2, img_coords->y1, img_coords->y2,
        //    clipped_img_area->x1, clipped_img_area->x2, clipped_img_area->y1, clipped_img_area->y2);
#endif
        ...
        while(blend_area.y1 <= y_last) {
            ...
            if(transformed) {
                //printf("relative_area [%d, %d, %d, %d]\n", relative_area.x1, relative_area.x2, relative_area.y1, relative_area.y2);
