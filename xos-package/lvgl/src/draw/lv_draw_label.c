                        ...
                        const lv_font_t * font, uint32_t letter, lv_draw_glyph_cb_t cb)
{
    ...
    if(g_ret == false) {
        ...
        #if defined(CONFIG_PRODUCT_STUDYPILOT) || defined(CONFIG_PRODUCT_STUDYPILOT_LH) || defined(CONFIG_PRODUCT_STUDY_LV)
        // 字库缺失时将占位符替换为星号:UTF-8 编码为 42
        letter = 42;
        lv_font_get_glyph_dsc(font, &g, letter, '\0');
        #endif
    }
    ...
    if(g.resolved_font) {
        ...
        if(LV_FONT_GLYPH_FORMAT_NONE < g.format && g.format < LV_FONT_GLYPH_FORMAT_IMAGE) {
            ...
            if(draw_buf == NULL) {
                ...
                if (w < 50) w = 50;
                if (h < 64) h = 64;
                draw_buf = lv_draw_buf_create(w, h, LV_COLOR_FORMAT_A8, LV_STRIDE_AUTO);
                LV_ASSERT_MALLOC(draw_buf);
                dsc->_draw_buf = lv_draw_buf_reshape(draw_buf, 0, g.box_w, g.box_h, LV_STRIDE_AUTO);
            }
        }
