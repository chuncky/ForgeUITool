...
static const char * image_src_type_name(lv_image_src_t src_type);
static void image_decoder_log_src(const char * msg, const void * src, lv_image_src_t src_type);
static void image_decoder_clear_open_result(lv_image_decoder_dsc_t * dsc);
static lv_result_t image_decoder_try_next_open(lv_image_decoder_dsc_t * dsc, lv_image_decoder_t * failed_decoder);
/**********************
 *  STATIC VARIABLES
 **********************/
...
lv_result_t lv_image_decoder_open(lv_image_decoder_dsc_t * dsc, const void * src, const lv_image_decoder_args_t * args)
{
    ...
    if (dsc->src_type == LV_IMAGE_SRC_VARIABLE) {
        dsc->decoded = (const lv_draw_buf_t *)src;
        if (dsc->decoded != NULL && dsc->decoded->phy_addr != 0 && dsc->decoded->header.stride != 0
            && dsc->decoded->header.cf >= LV_COLOR_FORMAT_RGB888)
            return LV_RESULT_OK;
        else
            dsc->decoded = NULL;
    }
    ...
    if(dsc->decoder == NULL) {
        image_decoder_log_src("lv_image_decoder_open: no decoder accepted", src, dsc->src_type);
    ...
    if(res != LV_RESULT_OK) {
        image_decoder_log_src("lv_image_decoder_open: decoder open failed", src, dsc->src_type);
        LV_LOG_WARN("lv_image_decoder_open: decoder=%p res=%d header=%" LV_PRIu32 "x%" LV_PRIu32
                    " cf=%d stride=%" LV_PRIu32,
                    (void *)dsc->decoder, (int)res, dsc->header.w, dsc->header.h,
                    (int)dsc->header.cf, dsc->header.stride);
        res = image_decoder_try_next_open(dsc, dsc->decoder);
    }
...
// printf("image_decoder_get_info ->in\n");
    _LV_LL_READ(img_decoder_ll_p, decoder) {
        ...
        if(decoder->info_cb && decoder->open_cb) {
            ...
            if(src_type == LV_IMAGE_SRC_FILE) {
                LV_LOG_INFO("lv_image_decoder_get_info: decoder=%p src=%s res=%d",
                            (void *)decoder, (const char *)src, (int)res);
            }
            if(res == LV_RESULT_OK) {
                if(header->stride == 0) {
...
{
    if(src_type == LV_IMAGE_SRC_FILE) {
        LV_LOG_WARN("%s src=%s type=%s", msg, (const char *)src, image_src_type_name(src_type));
    }
    else {
        LV_LOG_WARN("%s src=%p type=%s(%d)", msg, src, image_src_type_name(src_type), (int)src_type);
    }
}
...
static lv_result_t image_decoder_try_next_open(lv_image_decoder_dsc_t * dsc, lv_image_decoder_t * failed_decoder)
{
    ...
    image_decoder_clear_open_result(dsc);

    _LV_LL_READ(img_decoder_ll_p, decoder) {
        lv_image_header_t header;
        lv_result_t res;

        if(decoder == failed_decoder || decoder->info_cb == NULL || decoder->open_cb == NULL) continue;

        lv_memzero(&header, sizeof(header));
        res = decoder->info_cb(decoder, dsc->src, &header);
        if(dsc->src_type == LV_IMAGE_SRC_FILE) {
            LV_LOG_INFO("lv_image_decoder_open: fallback info decoder=%p src=%s res=%d",
                        (void *)decoder, (const char *)dsc->src, (int)res);
        }
        ...
        if(res == LV_RESULT_OK) {
            if(dsc->src_type == LV_IMAGE_SRC_FILE) {
                LV_LOG_WARN("lv_image_decoder_open: fallback decoder=%p opened src=%s header=%" LV_PRIu32
                            "x%" LV_PRIu32 " cf=%d stride=%" LV_PRIu32,
                            (void *)decoder, (const char *)dsc->src, dsc->header.w,
                            dsc->header.h, (int)dsc->header.cf, dsc->header.stride);
            }
            return LV_RESULT_OK;
        }

        if(dsc->src_type == LV_IMAGE_SRC_FILE) {
            LV_LOG_WARN("lv_image_decoder_open: fallback decoder=%p failed src=%s res=%d header=%" LV_PRIu32
                        "x%" LV_PRIu32 " cf=%d stride=%" LV_PRIu32,
                        (void *)decoder, (const char *)dsc->src, (int)res, dsc->header.w,
                        dsc->header.h, (int)dsc->header.cf, dsc->header.stride);
        }
        image_decoder_clear_open_result(dsc);
    }
