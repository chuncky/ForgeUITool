...
#if defined(CONFIG_PRODUCT_STUDYPILOT) || defined(CONFIG_PRODUCT_STUDY_LV)
#define SCROLL_ANIM_TIME_MIN    20     /*ms*/
...
void lv_obj_scroll_by(lv_obj_t * obj, int32_t dx, int32_t dy, lv_anim_enable_t anim_en)
{
    ...
    if(anim_en == LV_ANIM_ON) {
        ...
        if(dx) {
#if defined(CONFIG_PRODUCT_STUDYPILOT) || defined(CONFIG_PRODUCT_STUDY_LV)
            uint32_t t = lv_anim_speed_clamped((lv_display_get_horizontal_resolution(d)) * 6, SCROLL_ANIM_TIME_MIN,
...
uint32_t t = lv_anim_speed_clamped((lv_display_get_horizontal_resolution(d)) >> 1, SCROLL_ANIM_TIME_MIN,
        ...
        if(dy) {
#if defined(CONFIG_PRODUCT_STUDYPILOT) || defined(CONFIG_PRODUCT_STUDY_LV)
            uint32_t t = lv_anim_speed_clamped((lv_display_get_vertical_resolution(d)) * 6, SCROLL_ANIM_TIME_MIN,
                                               SCROLL_ANIM_TIME_MAX);
#else
