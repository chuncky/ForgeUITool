...
extern qua_mm_system_t *g_lvgl_mm_system;
#endif
...
#if !defined(BUILD_SIMULATOR) || BUILD_SIMULATOR == 0
    if (g_lvgl_mm_system != NULL) {
        qua_mm_system_ops_t *sys_ops = (qua_mm_system_ops_t *)g_lvgl_mm_system;
        sys_ops->sys_mmz_alloc("scroll_buffer", NULL, size, &phy_buffer, &virt_buffer);
    } else {
        LV_LOG_ERROR("lv_refr_now quamm system is not initialized!\n");
    }
#else
    virt_buffer = lv_malloc(size);
    phy_buffer = 0;
#endif
    if (virt_buffer) {
        lv_draw_buf_init(&obj->scroll_buf, w, h, cf, stride, virt_buffer, size);
        obj->scroll_buf.phy_addr = phy_buffer;
        obj->scroll_buf.cacheable = 0;
    } else {
        ...
        if(lv_obj_has_flag(obj, LV_OBJ_FLAG_USER_1)) {
            ...
            if (!obj->scroll_buf.unaligned_data) {
                ...
                if (w != obj->scroll_buf.header.w || h != obj->scroll_buf.header.h) {
                    ...
                    if (g_lvgl_mm_system != NULL) {
                        qua_mm_system_ops_t *sys_ops = (qua_mm_system_ops_t *)g_lvgl_mm_system;
                        sys_ops->sys_mmz_free(obj->scroll_buf.phy_addr, obj->scroll_buf.unaligned_data);
                    }
#else
                    if (obj->scroll_buf.unaligned_data) {
                        lv_free(obj->scroll_buf.unaligned_data);
                    }
#endif
