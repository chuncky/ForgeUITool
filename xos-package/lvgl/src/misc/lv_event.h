...
typedef enum {
    ...
    _LV_EVENT_FIRST = 0,          /** The First of default events*/
    ...
    LV_EVENT_NO_GESTURE_CLICKED,  /**< Called on release if not scrolled and no gesture */
    ...
    LV_EVENT_USER_KEY,             /**< A user key is sent to the object. Get the key with `lv_indev_get_key(lv_indev_active());`*/
    LV_EVENT_USER_LONG_PRESSED,     /**< Custom long pressed event for specific keys*/
    LV_EVENT_USER_LONG_PRESSED_RELEASE, /**< Custom long pressed release event for specific keys*/
